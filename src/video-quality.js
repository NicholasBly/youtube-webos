...
async function ensurePlaybackStarts() {
  ...
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    ...
    try {
      ...
      if (currentState === STATE_PLAYING) {
        ...
        if (DEBUG)
          console.info(
            '[VideoQuality] ✅ Playback verified! Kickstart complete.'
          );
        ...
        requestAnimationFrame(() => {
          ...
          if (isControlsVisible) {
            if (DEBUG)
              console.info(
                '[VideoQuality] 🎮 Controls are focused. Hiding and dismissing...'
              );

            const hideStyle = document.createElement('style');
            hideStyle.textContent =
              '.GLc3cc { opacity: 0 !important; transition: opacity 0.1s; }';
  ...
  if (DEBUG && !hasKickstarted) {
    console.warn(
      '[VideoQuality] ⚠️ Playback enforcer timed out without confirming PLAYING state.'
    );
  }
}
...
function setQualityOnPlayer() {
  ...
  try {
    ...
    DEBUG &&
      console.log(
        '[VideoQuality] Upgrading quality from:',
    ...
    DEBUG &&
      console.info(
        '[VideoQuality] ✅ Quality upgraded to:',
        afterQuality,
        qualityLabel
      );
...
function interceptAndUpgradeQuality(videoId) {
  ...
  DEBUG &&
    console.info(
      '[VideoQuality] 🛑 Intercepting playback to upgrade quality:',
      videoId
    );
  ...
  const attach = () => {
    ...
    try {
      ...
      try {
        ...
        DEBUG &&
          console.warn(
            '[VideoQuality] Event listener failed, falling back to poll:',
            e
          );
      }
...
function setupListeners() {
  ...
  window.addEventListener('ytaf-page-update', () => {
    handleNavigation({
      detail: { pageType: isWatchPage() ? 'watch' : 'other' }
    });
  });
