...
const VisibilityObserverClass =
  ...
  class {
    constructor(callback, options) {
      ...
      this.margin =
        options && options.rootMargin
          ? parseInt(options.rootMargin, 10) || 0
          : 0;
      this.interval = null;
    }
...
async function processUpgrade(element, generationId) {
  ...
  const applyUpgrade = (targetUrl, quality) => {
    requestAnimationFrame(() => {
      ...
      if (
        ...
        freshState &&
        freshState.generationId === generationId
      ) {
        ds.thumbVideoId = videoId;
  ...
  for (let i = 0; i < candidateQualities.length; i++) {
    const targetUrl = getThumbnailUrl(
      currentUrl,
      candidateQualities[i],
      pathMatch
    );
    if (targetUrl)
      candidates.push({ quality: candidateQualities[i], url: targetUrl });
  }
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          if (matchesFn && matchesFn.call(node, YT_THUMBNAIL_SELECTOR)) {
            ...
            for (let k = 0, kLen = nested.length; k < kLen; k++) {
              ...
              styleObserver.observe(targetNode, {
                attributes: true,
                attributeFilter: ['style']
              });
...
async function enableObserver() {
  ...
  const existingThumbnails = appContainer.querySelectorAll(
