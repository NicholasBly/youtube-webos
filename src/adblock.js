...
const RESPONSE_NEEDLE_RE =
  /responseContext|playerResponse|continuationContents/;
...
function recomputeFilterFlags() {
  cfgEmojiFixEffective =
    !!cfgSnapshot[CONFIG_KEYS.EMOJI_FIX] && cachedWebOSVersion <= 4;
...
function splitIntoRuns(text, originalRun = {}) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (i % 2 === 1) {
      newRuns.push(
        Object.assign({}, originalRun, { text: '\u200B' + parts[i] + '\u200C' })
      );
    } else {
...
function walkAndProcess(obj, doEmoji, doTracking, maxDepth, currentDepth = 0) {
  ...
  if (doTracking && typeof obj.trackingParams === 'string')
    obj.trackingParams = '';
...
const telemetryFetchHandler = (evt) => {
  ...
  if (TELEMETRY_REGEX.test(url.pathname)) {
    if (DEBUG)
      console.info('[AdBlock] Blocked telemetry Fetch request:', url.pathname);
    evt.preventDefault();
  }
};
...
export function initTrackingBlock() {
  ...
  try {
    if (typeof FetchRegistry !== 'undefined' && FetchRegistry.getInstance) {
      FetchRegistry.getInstance().addEventListener(
        'request',
        telemetryFetchHandler
      );
    }
  } catch (e) {
    console.warn(
      '[AdBlock] Fetch hook failed (expected behavior on webOS 3):',
      e.message
    );
  }
  ...
  try {
    ...
    window.XMLHttpRequest.prototype.send = function (_body) {
      ...
      if (reqUrl && TELEMETRY_REGEX.test(reqUrl)) {
        if (DEBUG)
          console.info('[AdBlock] Blocked telemetry XHR request:', reqUrl);
...
export function destroyTrackingBlock() {
  ...
  try {
    if (typeof FetchRegistry !== 'undefined' && FetchRegistry.getInstance) {
      FetchRegistry.getInstance().removeEventListener(
        'request',
        telemetryFetchHandler
      );
    }
    ...
    console.warn(
      '[AdBlock] Fetch unhook failed (expected on older engines):',
      e.message
    );
  }
...
function logSchemaMiss(data, textLength) {
  try {
    ...
    debugLog(
      `MISS (Fallback used) | Size: ${textLength} | Error analyzing structure`
    );
  }
}
...
function hookedParse(text, reviver) {
  ...
  if (!text || text.length < 500 || !data || typeof data !== 'object')
    return data;
  ...
  try {
    ...
    if (isShortsPage() && responseType && IGNORE_ON_SHORTS.has(responseType))
      return data;
  ...
  switch (responseType) {
      ...
      if (config.enableAdBlock && schema?.listPath) {
        ...
        if (Array.isArray(entries)) {
          ...
          if (DEBUG && entries.length !== oldLen)
            debugLog(
              `SHORTS_SEQUENCE: Removed ${oldLen - entries.length} items`
            );
        }
      }
      break;
    case 'HOME_BROWSE':
      if (schema?.mainContent) {
        ...
        if (!contents) {
          contents = findObjects(data, ['sectionListRenderer'], 8)
            .sectionListRenderer?.contents;
          if (DEBUG && contents)
            debugLog(`${responseType}: Using fallback search`);
        }
        ...
        if (Array.isArray(tabs)) {
          for (let i = 0; i < tabs.length; i++) {
            const gridContents =
              ...
              tabs[i].tabRenderer?.content?.tvSurfaceContentRenderer?.content
                ?.sectionListRenderer?.contents;
      ...
      if (schema?.gridPath) {
        ...
        if (Array.isArray(gridItems)) {
          ...
          if (DEBUG && oldLen !== gridItems.length)
            debugLog(
              `CONTINUATION (Grid): Removed ${oldLen - gridItems.length} items`
            );
        }
      }
      if (schema?.horizontalPath) {
        ...
        if (Array.isArray(horizItems)) {
          ...
          if (DEBUG && oldLen !== horizItems.length)
            debugLog(
              `CONTINUATION (Horizontal): Removed ${oldLen - horizItems.length} items`
            );
        }
      }
    ...
    case 'NEXT':
      if (config.enableAdBlock) {
        ...
        if (!overlay) {
          ...
          if (DEBUG && overlay)
            debugLog(
              `${responseType}: Path failed, found overlay via fallback`
            );
        }
        if (overlay?.timelyActionRenderers) {
          ...
          if (DEBUG)
            debugLog(
              `${responseType}: Removed timelyActionRenderers (QR Code)`
            );
        }
      }
      ...
      if (config.hideGuestPrompts) {
        ...
        if (!pivotContents) {
          pivotContents = findObjects(data, ['pivot'], 8).pivot
            ?.sectionListRenderer?.contents;
          if (DEBUG && pivotContents)
            debugLog(`${responseType}: Found pivot via fallback search`);
        }
      ...
      if (config.enableLegacyEmojiFix) {
        if (responseType === 'NEXT') {
          findAndProcessText(
            getByPath(data, ['contents', 'singleColumnWatchNextResults'])
          );
...
function applyFallbackFilters(data, config, needsContentFiltering) {
  ...
  if (
    config.enableAdBlock &&
    found.playerOverlayRenderer?.timelyActionRenderers
  ) {
  ...
  if (Array.isArray(found.entries)) {
    ...
    if (DEBUG && oldLen !== found.entries.length)
      debugLog(
        `FALLBACK (Entries): Removed ${oldLen - found.entries.length} items`
      );
  }
...
function getShelfTitleOptimized(shelf) {
  ...
  return (
    ...
    shelf.headerRenderer?.shelfHeaderRenderer?.avatarLockup
      ?.avatarLockupRenderer?.title?.runs?.[0]?.text ||
    ''
  );
}
...
function processSectionListOptimized(
  contents,
  config,
  needsContentFiltering,
  contextName = ''
  ...
  for (let i = 0; i < contents.length; i++) {
    ...
    if (item.shelfRenderer) {
      ...
      if (
        removeGlobalShorts &&
        shelf.tvhtml5ShelfRendererType === YT_CONSTANTS.SHELF_TYPE_SHORTS
      )
        keepItem = false;
      else if (removeGlobalShorts || removeTopLiveGames || removeMostRelevant) {
        const title = getShelfTitleOptimized(shelf);
        if (removeGlobalShorts && title === UI_STRINGS.SHORTS_TITLE)
          keepItem = false;
        else if (
          removeTopLiveGames &&
          title === UI_STRINGS.TOP_LIVE_GAMES_TITLE
        )
          keepItem = false;
        else if (removeMostRelevant && title === UI_STRINGS.MOST_RELEVANT_TITLE)
          keepItem = false;
      }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (needsContentFiltering) {
      if (
        ...
        hideGuestPrompts &&
        item.gridButtonRenderer?.title?.runs?.[0]?.text ===
          UI_STRINGS.GUEST_PROMPT_TEXT
      )
        keep = false;
    }
...
export function initAdblock() {
  ...
  if (DEBUG)
    console.info(
      '[AdBlock] Initializing hybrid hook (Debug Mode: ' + DEBUG + ')'
    );
