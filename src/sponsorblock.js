...
class SponsorBlockHandler {
  ...
  _isNodeConnected(node) {
    ...
    return node.isConnected !== undefined
      ? node.isConnected
  ...
  _syncOverlayPosition(ytPB) {
    ...
    const trackEl =
      this.progressBar && this.progressBar !== ytPB ? this.progressBar : ytPB;
  ...
  resetSegmentTracking() {
    ...
    this.nextSegmentStart =
      this.skipSegments.length > 0 ? this.skipSegments[0].start : Infinity;

    // Find the first segment that starts AFTER the current time, or contains current time.
    if (
      this.video &&
      !isNaN(this.video.currentTime) &&
      this.skipSegments.length > 0
    ) {
      ...
      if (currentIdx !== -1) {
        ...
        if (this.nextSegmentIndex < this.skipSegments.length) {
          this.nextSegmentStart =
            this.skipSegments[this.nextSegmentIndex].start;
        } else {
          this.nextSegmentStart = Infinity;
        }
      }
  ...
  buildSkipChain(segments) {
    ...
    if (chainParts.length === 1 && finalSeekTime - firstSeg.start < 1)
      return null;
  ...
  executeChainSkip(video) {
    ...
    if (video.readyState === 0) {
      if (this.boundChainSkipRetry && this.chainSkipVideo) {
        this.chainSkipVideo.removeEventListener(
          'loadedmetadata',
    ...
    this.skipSegments.forEach((seg) => {
      if (
        seg.mode === 'auto_skip' &&
        seg.start < chain.endTime &&
        seg.end <= chain.endTime + 0.1
      ) {
  ...
  async init() {
    ...
    try {
      ...
      const videoData = Array.isArray(data)
        ? data.find((x) => x.videoID === this.videoID)
        : data;
      ...
      this.segments = videoData.segments.sort(
        (a, b) => a.segment[0] - b.segment[0]
      );
      this.highlightSegment = this.segments.find(
        (s) => s.category === 'poi_highlight'
      );
  ...
  observePlayerUI() {
    ...
    const startOptimizedObserver = (targetNode) => {
      ...
      this.log(
        'info',
        'Attaching optimized observer to:',
        observeTarget.tagName
      );
  ...
  checkForProgressBar() {
    ...
    if (
      ...
      this._isNodeConnected(this.overlay.parentNode)
    ) {
  ...
  drawOverlay() {
    ...
    for (let i = 0; i < len; i++) {
      ...
      const color =
        config[colorKey] || segmentTypes[segment.category]?.color || '#00d400';
  ...
  handleTimeUpdate() {
    ...
    if (this.currentManualSegment) {
      if (
        currentTime < this.currentManualSegment.start ||
        currentTime >= this.currentManualSegment.end
      ) {
        this.clearManualNotification();
      }
    }
    ...
    if (
      this.tempWhitelistIndex !== -1 &&
      seg.originalIndex !== this.tempWhitelistIndex
    ) {
      this.tempWhitelistIndex = -1;
    }
    ...
    if (seg.mode === 'manual_skip') {
      if (this.currentManualSegment !== seg) {
        ...
        if (this.activeManualNotification)
          this.activeManualNotification.remove();
        this.activeManualNotification = showNotification(
          `${title}: Press Blue to skip`,
          0
        );
      }
      return;
    ...
    const config = configGetAll();
    if (
      config.skipSegmentsOnce &&
      this.skippedSegmentIndices.has(seg.originalIndex)
    ) {
      return;
    }
    ...
    if (
      segmentIdx === this.lastSkippedSegmentIndex &&
      Math.abs(currentTime - this.lastSkipTime) < 0.1
    ) {
      return;
    }
...
if (typeof window !== 'undefined') {
  ...
  if (document.readyState !== 'loading') {
    ...
    window.addEventListener('load', () => setTimeout(initSB, 500), {
      once: true
    });
  }
}
