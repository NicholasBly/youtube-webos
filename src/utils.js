  ...
  const initObserver = () => {
    ...
    const pageObserver = new MutationObserver((mutations) => {
      for (let m of mutations) {
        ...
        if (
          oldV.indexOf('WEB_PAGE_TYPE_') === -1 &&
          newV.indexOf('WEB_PAGE_TYPE_') === -1
        )
          continue;
        updatePageState();
        break;
...
export function isGuestMode() {
  ...
  try {
    const lastIdentity = window.localStorage.getItem(
      'yt.leanback.default::last-identity-used'
    );
    if (lastIdentity) {
      const parsed = JSON.parse(lastIdentity);
      if (
        parsed?.data?.identityType === 'UNAUTHENTICATED_IDENTITY_TYPE_GUEST'
      ) {
    ...
    const autoNav = window.localStorage.getItem(
      'yt.leanback.default::AUTONAV_FOR_LIVING_ROOM'
    );
    if (autoNav) {
...
export function sendKey(keyDef, target = document.body) {
  if (!keyDef?.code) {
    if (process.env.NODE_ENV !== 'production')
      console.warn('[Utils] Invalid key definition');
    return;
  }
  ...
  Object.defineProperties(keyDownEvt, {
    keyCode: codeDesc,
    which: codeDesc,
    charCode: charDesc
  });
  Object.defineProperties(keyUpEvt, {
    keyCode: codeDesc,
    which: codeDesc,
    charCode: charDesc
  });
...
export function handleLaunch(params) {
  ...
  if (typeof contentTarget === 'string') {
    if (contentTarget.startsWith(ytURL.origin)) {
      ...
      if (contentTarget.startsWith('v=v='))
