...
class PerfMonitor {
  ...
  startTracking() {
    ...
    try {
      const observer = new PerformanceObserver((list) => {
        for (const entry of list.getEntries()) {
          ...
          if (entry.attribution && entry.attribution.length > 0) {
            source =
              entry.attribution[0].name || entry.attribution[0].containerType;
          }
      ...
      console.warn(
        '[PerfMon] Long Task API not supported on this webOS version.'
      );
    }
      ...
      const resObserver = new PerformanceObserver((list) => {
        for (const entry of list.getEntries()) {
          if (entry.duration > 500 && !entry.name.includes('generate_204')) {
            ...
            this.stats.slowRequests.sort(
              (a, b) => parseFloat(b.duration) - parseFloat(a.duration)
            );
            if (this.stats.slowRequests.length > 4)
              this.stats.slowRequests.pop();
          }
        }
      });
  ...
  getWorstMutations() {
    const entries = Object.entries(this.mutationStats).map(
      ([caller, stats]) => ({ caller: caller, ...stats })
    );
