...
(function () {
  ...
  // Mirrors the legacy polyfill: ui.js sets keyMode = 'NONE' and calls
  // navigate() itself, so the global focus/mouse bookkeeping below only needs
  // to run while the host is actually going to navigate. The focusin listener
  // otherwise forces a layout on every focus change in the whole app.
  let trackFocus = true;
  ...
  function spatialNavigationHandler() {
    ...
    window.addEventListener('focusin', (e) => {
      // One forced layout per focus change across the entire app, to populate a
      // fallback only read inside navigate(). Skipped unless the host says it
      // will navigate.
      if (!trackFocus) return;
      if (e.target !== window) {
  ...
  function getInitialAPIs() {
    return {
      ...
      // Must exist before enableExperimentalAPIs() seals this object, or a host
      // assigning it gets "Cannot add property trackFocus, object is not
      // extensible" and the whole bundle dies at load.
