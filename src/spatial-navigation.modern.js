...
(function () {
  ...
  const SPINNABLE_INPUT_TYPES = new Set([
    'email',
    'date',
    'month',
    'number',
  ...
  function initiateSpatialNavigation() {
    ...
    window.Element.prototype.getSpatialNavigationContainer =
  ...
  function spatialNavigationHandler() {
    window.addEventListener('keydown', (e) => {
      const currentKeyMode =
        window.parent?.__spatialNavigation__?.keyMode ??
  ...
  function navigate(dir) {
    ...
    if (startingPoint) {
      const elementFromPosition =
        document.elementFromPoint(startingPoint.x, startingPoint.y) ??
        document.body;
      if (
        isFocusable(elementFromPosition) &&
        !isContainer(elementFromPosition)
      ) {
        startingPoint = null;
      } else {
        eventTarget = isContainer(elementFromPosition)
          ? elementFromPosition
          : elementFromPosition.getSpatialNavigationContainer();
      }
    }
    ...
    if (
      ...
      if (
        ...
        if (action === 'scroll' && scrollingController(eventTarget, dir))
          return;
        else if (action === 'focus') {
          const bestInsideCandidate = eventTarget.spatialNavigationSearch(dir, {
          ...
          if (
            focusingController(bestInsideCandidate, dir) ||
            scrollingController(eventTarget, dir)
          )
            return;
        }
      }
    ...
    let parentContainer = container.parentElement
      ? container.getSpatialNavigationContainer()
      : null;
  ...
  function focusingController(bestCandidate, dir) {
    if (bestCandidate) {
      ...
      bestCandidate.focus({
        preventScroll:
          container === window || getCSSSpatNavAction(container) !== 'focus'
      });
      startingPoint = null;
      return true;
    }
  ...
  function moveScroll(element, dir, offset = 0) {
    ...
    switch (dir) {
      case 'left':
        element.scrollBy({ left: -scrollStep });
        break;
      case 'right':
        element.scrollBy({ left: scrollStep });
        break;
      case 'up':
        element.scrollBy({ top: -scrollStep });
        break;
      case 'down':
        element.scrollBy({ top: scrollStep });
        break;
  ...
  function getSpatialNavigationCandidates(
    ...
    if (container.childElementCount > 0) {
      if (!container.parentElement)
        container = container.querySelector('body') ?? document.body;
  ...
  function getFilteredSpatialNavigationCandidates(
    ...
    candidates =
      !candidates || candidates.length === 0
        ? getSpatialNavigationCandidates(container)
        : candidates;
  ...
  function spatialNavigationSearch(dir, args = {}) {
    ...
    if (args.container && defaultContainer.contains(args.container)) {
      ...
      for (let i = 0; i < additional.length; i++)
        defaultCandidates.push(additional[i]);
    }

    const rawCandidates = args.candidates?.length
      ? args.candidates
      : defaultCandidates;
    const candidates = [];
    ...
    for (let i = 0; i < rawCandidates.length; i++) {
      ...
      if (container.contains(c) && (!isDefault || container !== c))
        candidates.push(c);
    }
    ...
    for (let i = 0; i < insideOverlappedCandidates.length; i++) {
      ...
      if (!internalSet.has(c) && container.contains(c))
        internalCandidates.push(c);
    }
    ...
    if (internalCandidates.length && targetElement.nodeName !== 'INPUT') {
      bestTarget = selectBestCandidateFromEdge(
        targetElement,
        internalCandidates,
        dir
      );
    ...
    if (bestTarget && isDelegableContainer(bestTarget)) {
      ...
      const descendantsBest = innerTarget.length
  ...
  function filteredCandidates(currentElm, candidates, dir, container) {
    ...
    const eventTargetRect =
      originalContainer.parentElement &&
      container !== originalContainer &&
      !isVisible(currentElm)
        ? getBoundingClientRect(originalContainer)
        : searchOriginRect || getBoundingClientRect(currentElm);

    const isCurrentContainer =
      (isContainer(currentElm) || currentElm.nodeName === 'BODY') &&
      currentElm.nodeName !== 'INPUT';
    const result = [];
  ...
  function selectBestCandidate(currentElm, candidates, dir) {
    ...
    const isGrid =
      getCachedComputedStyle(container)
    ...
    if (isGrid) {
      ...
      for (let i = 0; i < candidates.length; i++) {
        if (
          isAligned(
            currentTargetRect,
            getBoundingClientRect(candidates[i]),
            dir
          )
        )
          aligned.push(candidates[i]);
      }
    ...
    return getClosestElement(
      ...
      candidates,
      dir,
      isGrid ? getAbsoluteDistance : getDistance
    );
  }
  ...
  function selectBestCandidateFromEdge(currentElm, candidates, dir) {
    return getClosestElement(
      ...
      candidates,
      dir,
      startingPoint ? getDistanceFromPoint : getInnerDistance
    );
  }
  ...
  function getClosestElement(currentElm, candidates, dir, distanceFunction) {
    ...
    return minDistanceElements.length > 1 &&
  ...
  function getScrollContainer(element) {
    ...
    do {
      ...
      !isScrollContainer(scrollContainer) ||
      !isVisible(scrollContainer)
    );

    return scrollContainer === document ||
  ...
  function focusableAreas(option = { mode: 'visible' }) {
    ...
    for (let i = 0; i < elements.length; i++) {
      const el = elements[i];
      if (isFocusable(el) && (option.mode === 'all' || isVisible(el)))
        result.push(el);
    }
    return result;
  }
  ...
  function createSpatNavEvents(
    ...
    if (eventType === 'beforefocus' || eventType === 'notarget') {
      return containerElement.dispatchEvent(
        new CustomEvent('nav' + eventType, {
          bubbles: true,
          cancelable: true,
          detail: { causedTarget: currentElement, dir: direction }
        })
      );
    }
  }
  ...
  function readCssVar(element, varName) {
    return (
      getCachedComputedStyle(element).getPropertyValue(`--${varName}`) || ''
    ).trim();
  }
  ...
  function navigateChain(eventTarget, container, parentContainer, dir, option) {
    ...
    while (parentContainer) {
      if (
        focusingController(
          eventTarget.spatialNavigationSearch(dir, currentOption),
          dir
        )
      )
        return;
    ...
    if (getCSSSpatNavAction(container) === 'auto' && option === 'visible')
      scrollingController(container, dir);
  }
  ...
  function findSearchOrigin() {
    ...
    if (
      !searchOrigin ||
      (searchOrigin === document.body && !document.querySelector(':focus'))
    ) {
      if (
    ...
    if (!isVisibleInScroller(searchOrigin)) {
      ...
      if (
        scroller &&
        (scroller === window || getCSSSpatNavAction(scroller) === 'auto')
      )
        return scroller;
    }
    return searchOrigin;
  ...
  function isScrollContainer(element) {
    ...
    return (
      (style.overflowX !== 'visible' &&
        ...
        isOverflow(element, 'left')) ||
      (style.overflowY !== 'visible' &&
        style.overflowY !== 'clip' &&
        isOverflow(element, 'down'))
    );
  }
  ...
  function isVisibleInScroller(element) {
    ...
    return (
      isInside(scrollerRect, elementRect, 'left') &&
      isInside(scrollerRect, elementRect, 'down')
    );
  }
  ...
  function isVisible(element) {
    return (
      !element.parentElement ||
      (isVisibleStyleProperty(element) && hitTest(element))
    );
  }
  ...
  function isEntirelyVisible(element, container) {
    ...
    const containerRect = getBoundingClientRect(
      container || element.getSpatialNavigationContainer()
    );
    return !(
  ...
  function hitTest(element) {
    ...
    if (
      ...
      (rect.top < 0 ||
        rect.left < 0 ||
        rect.top > docElm.clientHeight ||
        rect.left > docElm.clientWidth)
    )
      return false;
    ...
    for (let i = 0; i < 3; i++) {
      const elemFromPoint = element.ownerDocument.elementFromPoint(
        points[i][0],
        points[i][1]
      );
  ...
  function getInnerDistance(rect1, rect2, dir) {
    const edge = { left: 'right', right: 'left', up: 'bottom', down: 'top' }[
      dir
    ];
  ...
  function getDistance(searchOrigin, candidateRect, dir) {
    ...
    if (dir) {
      const alignBias = isAligned(searchOrigin, candidateRect, dir)
        ? Math.min(
            intersectionRect[isLR ? 'height' : 'width'] /
              searchOrigin[isLR ? 'height' : 'width'],
            1
          )
        : 0;
      const orthogonalBias =
        alignBias > 0 ? 0 : searchOrigin[isLR ? 'height' : 'width'] / 2;
  ...
  function getEntryAndExitPoints(dir = 'down', searchOrigin, candidateRect) {
    ...
    if (startingPoint) {
      ...
      if (dir === 'left' || dir === 'right') {
        points.entryPoint.y = Math.max(
          candidateRect.top,
          Math.min(startingPoint.y, candidateRect.bottom)
        );
      } else {
        points.entryPoint.x = Math.max(
          candidateRect.left,
          Math.min(startingPoint.x, candidateRect.right)
        );
      }
    } else {
      if (dir === 'left') {
      ...
      if (dir === 'left' || dir === 'right') {
        if (isBelow(searchOrigin, candidateRect)) {
          ...
          points.exitPoint.y = points.entryPoint.y = Math.max(
            searchOrigin.top,
            candidateRect.top
          );
        }
  ...
  function getIntersectionRect(rect1, rect2) {
    ...
    const area =
      maxLeft < minRight && maxTop < minBottom ? Math.sqrt(width * height) : 0;
  ...
  function handlingEditableElement(e) {
    ...
    if (
      SPINNABLE_INPUT_TYPES.has(target.type) &&
      (dir === 'up' || dir === 'down')
    ) {
  ...
  function getOverlappedCandidates(targetElement) {
    const areas = targetElement
      .getSpatialNavigationContainer()
      .focusableAreas();
    const result = [];
    for (let i = 0; i < areas.length; i++) {
      if (
        targetElement !== areas[i] &&
        isEntirelyVisible(areas[i], targetElement)
      )
        result.push(areas[i]);
    }
    return result;
  ...
  function getExperimentalAPI() {
    ...
    function findTarget(findCandidate, element, dir, option) {
      ...
      while (parentContainer) {
        ...
        if (candidates?.length > 0) {
          const bestNextTarget = eventTarget.spatialNavigationSearch(dir, {
            candidates,
            container
          });
          if (bestNextTarget)
    ...
    return {
      ...
      findCandidates: (element, dir, option) =>
        findTarget(true, element, dir, option),
      findNextTarget: (element, dir, option) =>
        findTarget(false, element, dir, option),
      getDistanceFromTarget: (element, candidateElement, dir) => {
        if (
          (isContainer(element) || element.nodeName === 'BODY') &&
  ...
  function getInitialAPIs() {
    return {
      ...
      set keyMode(mode) {
        this._keymode = ['SHIFTARROW', 'ARROW', 'NONE'].includes(mode)
          ? mode
          : 'ARROW';
      },
  ...
  function enableExperimentalAPIs(option) {
    ...
    window.__spatialNavigation__ =
      option === false
        ? getInitialAPIs()
        : Object.assign(getInitialAPIs(), getExperimentalAPI());
