...
class ReturnYouTubeDislike {
  ...
  async fetchVideoData() {
    ...
    try {
      ...
      const response = await Promise.race([
        fetch(
          `https://returnyoutubedislikeapi.com/votes?videoId=${this.videoID}`,
          fetchOptions
        ),
  ...
  refreshMenuCache() {
    ...
    let rawItems = [].slice.call(
      this.panelElement.querySelectorAll(SELECTORS.menuItem)
    );
    // Filter nested items
    this.menuItemsCache = rawItems.filter(
      (item) => !item.querySelector(SELECTORS.menuItem)
    );
    ...
    // Reset index
    this.focusedIndex = -1;
    const currentFocused = this.panelElement.querySelector(
  ...
  updateVisualState(targetItem) {
    ...
    if (
      !itemToClear &&
      this.focusedIndex !== -1 &&
      this.menuItemsCache[this.focusedIndex]
    ) {
    ...
    if (itemToClear && itemToClear !== targetItem) {
      itemToClear.classList.remove(
        SELECTORS.legacyHighlight,
        SELECTORS.focusState
      );
  ...
  clearAllHighlights() {
    ...
    if (this.lastFocusedElement) {
      this.lastFocusedElement.classList.remove(
        SELECTORS.focusState,
        SELECTORS.legacyHighlight
      );
  ...
  handleNavigation(e) {
    ...
    if (
      ...
      if (this.lastFocusedElement) {
        this.focusedIndex =
          this.menuItemsMap.get(this.lastFocusedElement) ?? -1;
    ...
    const nextIndex = isDown
      ? (this.focusedIndex + 1) % this.menuItemsCache.length
      : (this.focusedIndex - 1 + this.menuItemsCache.length) %
        this.menuItemsCache.length;
